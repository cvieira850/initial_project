...
describe('AccountPgRepository', () => {
  ...
  describe('UpdateAccountRole', () => {
    ...
    it('Should return undefined if updateAccountRole dont return an user', async () => {
      const user = await sut.updateAccountRole({ id: '1', roleId: '2' })

      expect(user).toBeUndefined()
    })

    it('Should return undefined if updateAccountRole dont return a role', async () => {
      await pgRoleRepo.save({ name: 'user', weight: 1 })
      await pgUserRepo.save({ email: 'any_email', name: 'any_name', password: '1234', role_id: '1' })
  ...
  describe('updateResetPasswordToken', () => {
    it('Should return an account on updateResetPasswordToken success', async () => {
      await pgRoleRepo.save({ name: 'user', weight: 1 })
      await pgUserRepo.save({ email: 'any_email', name: 'any_name', password: '1234', role_id: '1' })

      const user = await sut.updateResetPasswordToken({ id: '1', token: 'any_token' })

      expect(user).toEqual({ id: '1', name: 'any_name', role: 'user', email: 'any_email', password: '1234', reset_password_token: 'any_token' })
    })

    it('Should return undefined if updateResetPasswordToken dont return an user', async () => {
      const user = await sut.updateResetPasswordToken({ id: '1', token: 'any_token' })
  ...
  describe('loadByResetToken', () => {
    it('Should return an account on loadByResetToken success', async () => {
      await pgRoleRepo.save({ name: 'user', weight: 1 })
      await pgUserRepo.save({ email: 'any_email', name: 'any_name', password: '1234', role_id: '1', reset_password_token: 'any_token' })

      const user = await sut.loadByResetToken({ token: 'any_token' })

      expect(user).toEqual({ id: '1', name: 'any_name', email: 'any_email' })
    })

    it('Should return undefined if loadByResetToken dont return an user', async () => {
      const user = await sut.loadByResetToken({ token: 'any_token' })
